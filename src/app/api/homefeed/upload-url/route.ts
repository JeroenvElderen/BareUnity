...
export async function POST(request: Request) {
  ...
  if (!isSupabaseAdminConfigured) {
    return NextResponse.json(
      { error: "Image uploads are unavailable. Supabase is not configured." },
      { status: 503 },
    );
  }
  ...
  if (!IMAGE_UPLOAD_TYPES.has(contentType)) {
    return NextResponse.json(
      {
        error:
          "Unsupported image type. Please upload a JPG, PNG, WEBP, GIF, or AVIF image.",
      },
      { status: 400 },
    );
  }
  ...
  if (!size || size > MAX_UPLOAD_BYTES) {
    return NextResponse.json(
      ...
      {
        status: size > MAX_UPLOAD_BYTES ? 413 : 400,
      },
    );
  }
  ...
  if (error) {
    return NextResponse.json(
      {
        error: `Could not prepare image upload: ${error.message}`,
      },
      { status: 503 },
    );
  }
