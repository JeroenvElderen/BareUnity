...
export async function POST(request: NextRequest, context: RouteContext) {
  ...
  const { data: reply, error: replyError } = await supabaseAdmin
    ...
    .insert({
      ...
      author_id: adminResult.user.id,
      author_email: adminResult.user.email ?? null,
      author_role: "admin",
  ...
  await supabaseAdmin.from("feedback_messages").update({ status: "answered" }).eq("id", feedbackId);
