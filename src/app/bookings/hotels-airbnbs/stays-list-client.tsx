...
export function StaysListClient({ listings }: StaysListClientProps) {
  ...
  const countryOptions = useMemo(() => {
    const counts = liveListings.reduce((accumulator, listing) => {
      accumulator.set(
        listing.country,
        (accumulator.get(listing.country) ?? 0) + 1,
      );
      return accumulator;
    ...
    return Array.from(
      counts,
      ([country, count]): CountryOption => ({ country, count }),
    ).sort((a, b) => {
  ...
  const [selectedTypes, setSelectedTypes] =
    useState<Record<string, boolean>>(emptyTypeFilters);

  const hasTypeFilters = Object.values(selectedTypes).some(Boolean);
  const hasActiveFilters =
    selectedCountry.length > 0 || ratingFilter !== null || hasTypeFilters;

  const results = useMemo(() => {
    const filtered = liveListings.filter((listing) => {
      const matchesCountry =
        selectedCountry.length === 0 || listing.country === selectedCountry;
      const matchesType =
        !hasTypeFilters || (selectedTypes[listing.type] ?? false);
      const matchesRating =
        ratingFilter === null ||
        ...
        <aside className={styles.filters}>
          <div className={styles.filterCard}>
            ...
            <p className={styles.filterHint}>
              Narrow results instantly as you choose options.
            </p>
