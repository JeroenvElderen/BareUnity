...
export default function ProfileSetupCard({ user }: { user: User }) {
  ...
  return (
    <section className="rounded-2xl border border-accent/20 bg-card/75 p-4 shadow-[0_20px_45px_-35px_rgba(0,0,0,0.95)] backdrop-blur">
      <div className="mb-3">
        <h2 className="text-lg font-bold text-text">Profile setup</h2>
        <p className="text-sm text-text/70">Finish your profile with a banner, bio, and verification details.</p>
      </div>

      <div className="space-y-3">
        <div>
          <label className="text-sm font-semibold text-text">Profile banner URL</label>
          <input
            type="url"
            value={draft.bannerUrl}
            onChange={(e) => setDraft((current) => ({ ...current, bannerUrl: e.target.value }))}
            placeholder="https://example.com/banner.jpg"
            className="mt-1 w-full rounded-lg border border-accent/20 bg-accent/80 px-3 py-2 text-sm text-text/90 outline-none placeholder:text-text-inverse/50 focus:ring-2 focus:ring-sand/35"
          />
        </div>

        {draft.bannerUrl && (
          <div
            className="h-24 rounded-xl border border-accent/20 bg-cover bg-center"
            style={{ backgroundImage: `url(${draft.bannerUrl})` }}
          />
        )}

        <div>
          <label className="text-sm font-semibold text-text">Profile bio</label>
          <textarea
            value={draft.bio}
            onChange={(e) => setDraft((current) => ({ ...current, bio: e.target.value }))}
            placeholder="Tell people about yourself"
            className="mt-1 min-h-24 w-full rounded-lg border border-accent/20 bg-accent/80 px-3 py-2 text-sm text-text/90 outline-none placeholder:text-text-inverse/50 focus:ring-2 focus:ring-sand/35"
          />
        </div>
        ...
        <button
          type="button"
          onClick={saveDraft}
          className="rounded-xl border border-sand/25 bg-accent/15 px-4 py-2 text-sm font-semibold text-text transition hover:bg-accent/30"
        >
