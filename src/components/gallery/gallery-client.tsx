...
const FULLSCREEN_INSTRUCTIONS_ACK_KEY =
  "gallery-fullscreen-instructions-acknowledged";
...
async function fetchGallerySnapshot(
  ...
  const response = await fetch(`/api/gallery/snapshot?gallery=${gallery}`, {
    cache: "no-store",
    headers,
  });
...
export function GalleryClient({
  ...
  const [
    hasAcknowledgedSwipeInstructions,
    setHasAcknowledgedSwipeInstructions,
  ] = useState(false);
  ...
  const writeItemsToCache = useCallback(
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(
      refreshVisibleGallery,
      GALLERY_VISIBLE_REFRESH_INTERVAL_MS,
    );
  ...
  useEffect(() => {
    const hasAcknowledged =
      window.localStorage.getItem(FULLSCREEN_INSTRUCTIONS_ACK_KEY) === "true";
  ...
  const handleUpload: ChangeEventHandler<HTMLInputElement> = async (event) => {
    ...
    try {
      ...
      if (
        ...
        setUploadError(
          "Upload received and routed for moderation. It will appear after approval in the correct gallery.",
        );
      }
      ...
      for (let attempt = 0; attempt < UPLOAD_RECONCILE_ATTEMPTS; attempt += 1) {
        ...
        if (
          !uploadedItem ||
          nextItems.some((item) => item.path === uploadedItem.path)
        ) {
          break;
        }
  ...
  const toggleLike = async (itemId: string) => {
    ...
    const optimisticCount = Math.max(
      0,
      target.likeCount + (nextLiked ? 1 : -1),
    );
  ...
  const reportMedia = async (item: GalleryItem) => {
    ...
    void refreshGallery().then((nextItems) => {
      ...
      if (
        activeItem &&
        !nextItems.some((nextItem) => nextItem.path === activeItem.path)
      ) {
        setActiveItem(null);
      }
    });
      ...
      <section className={styles.wrapper}>
        ...
        {!isLoading && items.length === 0 ? (
          <div className={styles.emptyState}>
            <p>No approved media found in this gallery yet.</p>
            <small>
              Upload an image with + (automatically converted to WEBP) or add
              files to media storage.
            </small>
          </div>
        ) : (
            ...
            {items.map((item, index) => (
              <figure
                ...
                className={`${styles.tile} ${
                  styles[
                    TILE_SIZE_VARIANTS[
                      hashString(item.id) % TILE_SIZE_VARIANTS.length
                    ]
                  ]
                }`}
              >
          ...
          {showSwipeInstructions ? (
            ...
            >
              ...
              <p className={styles.swipeInstructionsBody}>
                Swipe left or right to move between photos. <br></br>Tap
                anywhere to close fullscreen.
              </p>
              <button
                type="button"
                ...
                onClick={() => {
                  window.localStorage.setItem(
                    FULLSCREEN_INSTRUCTIONS_ACK_KEY,
                    "true",
                  );
