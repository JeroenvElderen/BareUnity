...
export async function sendFriendRequestToProfile(target: {
  id?: string | null;
  username?: string | null;
  ...
  if (!viewer) {
    return {
      ...
      message: "Sign in to send a friend request.",
    };
  }

  if (!(await canCurrentUserAct(viewer.id))) {
    return { ok: false, reason: "blocked", message: VIEW_ONLY_ACTION_MESSAGE };
  }
  ...
  const receiverProfile = await resolveTargetProfile({
    userId: target.id,
  ...
  if (!receiverProfile) {
    return {
      ...
      message: "Could not find that member profile.",
    };
  }
  ...
  if (receiverProfile.allow_friend_requests === false) {
    return {
      ok: false,
      reason: "blocked",
      message: "This member is not accepting requests right now.",
    };
  }

  const [existingFriendship, existingRequest, senderProfile] =
    await Promise.all([
      supabase
        .from("friendships")
        .select("id")
  ...
  if (existingFriendship.data?.id) {
    return {
      ...
      message: `You're already connected with @${receiverProfile.username}.`,
    };
  }
  ...
  if (error) {
    ...
    return {
      ok: false,
      reason: "failed",
      message: "Could not send the friend request right now.",
    };
  }

  return {
    ok: true,
    message: `Friend request sent to @${receiverProfile.username}.`,
  };
}
